...
function teamcity(runner) {
	...
	runner.on('suite end', function (suite) {
		log('这是：：：：：：',flowId);
		if (suite.root) return;
		log('##teamcity[publishArtifacts name=\'' + escape(suite.title) + '\' duration=\'' + (new Date() - suite.startDate) + '\' flowId=\'' + flowId + '\']');
	});

	runner.on('end', function () {
		var duration;
		(typeof stats === 'undefined') ? duration = '0' : duration = stats.duration;
		log('##teamcity[publishArtifacts name=\'mocha.suite\' duration=\'' + duration + '\' flowId=\'' + flowId + '\']');
	});
}
